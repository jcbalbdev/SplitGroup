...
export function MemberList({ members, memberGroupsMap, nicknames, currentUserEmail, avatarVersion, onEditNickname, onAvatarClick, dn }) {
  return (
    <div className="animate-fade-in">
      <p className="text-xs text-muted" style={{ marginBottom: 10, paddingLeft: 4, display: 'flex', alignItems: 'center', gap: 4 }}>
        Toca <Pencil size={12} /> para ponerle un apodo · Los badges muestran grupos en común
      </p>
      <div className="list">
        {members.map((m) => {
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                {isMe ? (
                  <div style={{ position: 'relative', flexShrink: 0, cursor: 'pointer' }} onClick={onAvatarClick} title="Personalizar mi avatar">
                    ...
                    <div style={{ position: 'absolute', bottom: -2, right: -2, width: 18, height: 18, borderRadius: '50%', background: 'var(--primary)', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '2px solid var(--bg-card)' }}>
                ...
                <button className="btn btn-ghost btn-icon" title="Editar apodo"
                  ...
                  style={{ color: 'var(--text-muted)', padding: '6px 8px', flexShrink: 0 }}><Pencil size={15} /></button>
              </div>
